...
namespace Université_Domain.UseCases.NoteUseCase.Update;

public class UpdateEtudiantNote(IRepositoryFactory noterepository)
{
    public async Task<List<Note>> ExecuteAsync(long numEtud,long ueId, float valeur , float valeur2)
    {
        ...
        await noterepository.NoteRepository().UpdateAsync(notes);
